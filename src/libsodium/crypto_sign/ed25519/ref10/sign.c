                              ...
                              const unsigned char *sk, int prehashed)
{
    ...
    unsigned char            pk[32];
    unsigned char            sk_hash[64];
    unsigned char            nonce[64];
    unsigned char            hram[64];
    ge25519_p3               R;

    crypto_derive_public_from_secret_impl((unsigned char *)sk, pk);
    crypto_generichash_blake2b_init(&hs, NULL, 0, 64);
#ifdef ED25519_NONDETERMINISTIC
    memcpy(az, sk, 32);
    _crypto_sign_ed25519_synthetic_r_hv(&hs, nonce, sk_hash);
#else
    crypto_generichash_blake2b(sk_hash, 64, sk, 32, NULL, 0);
    crypto_generichash_blake2b_update(&hs, sk_hash + 32, 32);
#endif
    ...
    sc25519_reduce(hram);
    _crypto_sign_ed25519_clamp(sk_hash);
    sc25519_muladd(sig + 32, hram, sk_hash, nonce);
